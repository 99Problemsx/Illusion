...
EliteBattle.defineMoveAnimation(:MUDSHOT) do
  ...
  for k in 0...64
    ...
    fp[i.to_s].bitmap = Bitmap.new(bmp.width,bmp.height)
    fp[i.to_s].bitmap.blt(0,0,bmp,Rect.new(0,0,bmp.width,bmp.height))
    fp[i.to_s].ox = fp[i.to_s].bitmap.width/2
    fp[i.to_s].oy = fp[i.to_s].bitmap.height/2
    fp[i.to_s].opacity = 0
    fp[i.to_s].z = 19
    fp[i.to_s].color = Color.new(248,248,248,200)
  ...
  for i in 0...64
    ...
    for j in 0...64
      if fp[j.to_s].opacity == 0
        ...
        fp[j.to_s].x = dx[j]
        fp[j.to_s].y = dy[j]
        fp[j.to_s].zoom_x = 0.8#(!@targetIsPlayer ? 1.2 : 0.8)#@userSprite.zoom_x
        fp[j.to_s].zoom_y = 0.8#(!@targetIsPlayer ? 1.2 : 0.8)#@userSprite.zoom_y
        fp[j.to_s].opacity = 128 if !(j>i*2)
      end
      next if j>(i*2)
      ...
      fp[j.to_s].zoom_x += 0.04#(factor - fp["#{j}"].zoom_x)*0.2
      fp[j.to_s].zoom_y += 0.04#(factor - fp["#{j}"].zoom_y)*0.2
      fp[j.to_s].opacity += 32
      fp[j.to_s].angle += 8*(rangl[j]==0 ? -1 : 1)
      fp[j.to_s].angle = -Math.atan(1.0*(y2-y0)/(x2-x0))*180/Math::PI + (rand(4)==0 ? 180 : 0)
      fp[j.to_s].color.alpha -= 5 if fp[j.to_s].color.alpha > 0
  ...
  for j in 0...48; fp[j.to_s].visible = false; end
