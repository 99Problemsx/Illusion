...
EliteBattle.defineMoveAnimation(:HEALBLOCK) do
  ...
  for i in 0...sparks
    ...
    fp[i.to_s].bitmap = pbBitmap("Graphics/EBDX/Animations/Moves/eb619_3")
  ...
  for i in 0...128
    ...
    for j in 0...sparks
      if fp[j.to_s].opacity == 0
        ...
        fp[j.to_s].x = cx - 64*factor*@targetSprite.zoom_x + x*@targetSprite.zoom_x
        fp[j.to_s].y = cy - 64*factor*@targetSprite.zoom_y + y*@targetSprite.zoom_y
      end
      next if j>(i/4)
      x2 = cx
      y2 = cy
