                                                        ...
                                                        proc { |score, move, user, target, ai, battle|
                                                          if ai.trainer.high_skill? && target.faster_than?(user) && move.rough_priority(user) > 0
                                                            ...
                                                            ai.each_foe_battler(user.side) do |b, i|
                                                              if !b.has_move_with_function?("ProtectUserSideFromPriorityMoves")
                                                                next
                                                              end
                                                              if Settings::MECHANICS_GENERATION <= 5 && b.effects[PBEffects::ProtectRate] > 1
                                                                next
                                                              end
                                                              score -= 5
                                                            end
...
Battle::AI::Handlers::GeneralMoveScore.add(:damaging_move_and_either_side_no_reserves,
                                           proc { |score, move, user, ai, battle|
                                             if ai.trainer.medium_skill? && move.damagingMove?
                                               ...
                                               # AI is outnumbered
                                               next score if ai.trainer.high_skill? && foes > reserves
                                                        ...
                                                        proc { |score, move, user, target, ai, battle|
                                                          if move.damagingMove?
                                                            ...
                                                            if target.effects[PBEffects::Substitute] > 0
                                                              target.effects[PBEffects::Substitute]
...
Battle::AI::Handlers::GeneralMoveAgainstTargetScore.add(:external_flinching_effects,
                                                        proc { |score, move, user, target, ai, battle|
                                                          if ai.trainer.medium_skill? && move.damagingMove? && !move.move.flinchingMove? &&
                                                             user.faster_than?(target) && target.effects[PBEffects::Substitute] == 0 && (user.has_active_item?([:KINGSROCK, :RAZORFANG]) ||
                                                               user.has_active_ability?(:STENCH)) && (battle.moldBreaker || !target.has_active_ability?([:INNERFOCUS, :SHIELDDUST]))
                                                            old_score = score
                                                            score += 8
...
Battle::AI::Handlers::GeneralMoveAgainstTargetScore.add(:thawing_move_against_frozen_target,
                                                        proc { |score, move, user, target, ai, battle|
                                                          if ai.trainer.medium_skill? && target.status == :FROZEN && (move.rough_type == :FIRE || (Settings::MECHANICS_GENERATION >= 6 && move.move.thawsUser?))
                                                            old_score = score
                                                            score -= 20
...
Battle::AI::Handlers::GeneralMoveAgainstTargetScore.add(:trigger_target_ability_or_item_upon_hit,
                                                        proc { |score, move, user, target, ai, battle|
                                                          if ai.trainer.high_skill? && move.damagingMove? && target.effects[PBEffects::Substitute] == 0
                                                            if target.ability_active? && (Battle::AbilityEffects::OnBeingHit[target.ability] ||
                                                            ...
                                                            if target.item_active? && (Battle::ItemEffects::OnBeingHit[target.item] ||
...
Battle::AI::Handlers::GeneralMoveAgainstTargetScore.add(:trigger_user_ability_upon_hit,
                                                        proc { |score, move, user, target, ai, battle|
                                                          ...
                                                          if ai.trainer.high_skill? && user.ability_active? && move.damagingMove? &&
                                                                                                                       target.effects[PBEffects::Substitute] == 0 && ((Battle::AbilityEffects::OnDealingHit[user.ability] && move.move.contactMove?) ||
...
Battle::AI::Handlers::GeneralMoveAgainstTargetScore.add(:knocking_out_a_destiny_bonder_or_grudger,
                                                        proc { |score, move, user, target, ai, battle|
                                                          if (ai.trainer.has_skill_flag?("HPAware") || ai.trainer.high_skill?) && move.damagingMove? &&
                                                            ...
                                                            priority = move.rough_priority(user)
                                                            if (priority > 0 || (priority == 0 && user.faster_than?(target))) && (move.rough_damage > target.hp * 1.1)   # Predicted to KO the target
                                                              old_score = score
...
Battle::AI::Handlers::GeneralMoveAgainstTargetScore.add(:damaging_a_raging_target,
                                                        proc { |score, move, user, target, ai, battle|
                                                          if ai.trainer.medium_skill? && target.effects[PBEffects::Rage] && move.damagingMove?
                                                            ...
                                                            if ai.trainer.has_skill_flag?("HPAware") && ((move.rough_damage + target.rough_end_of_round_damage) * 2 > target.hp * 1.1)
                                                              next score
                                                              end
                                                            old_score = score
                                                            score -= 10
                                                          ...
                                                          if ai.trainer.medium_skill? && target.effects[PBEffects::Bide] > 0 && move.damagingMove?
                                                            ...
                                                            if !Effectiveness.ineffective?(eff)
                                                              ...
                                                              if ai.trainer.has_skill_flag?("HPAware")
                                                                ...
                                                                if (dmg * hits_possible) + eor_dmg > target.hp * 1.1
                                                                  next score
                                                                end
                                                              end
                                                              old_score = score
