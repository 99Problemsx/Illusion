...
class Battle::AI
  ...
  def pbChooseToSwitchOut(terrible_moves = false)
    ...
    if terrible_moves
      ...
      if should_switch && @trainer.medium_skill? && Battle::AI::Handlers.should_not_switch?(@user, reserves, self, @battle)
        should_switch = false
      end
  ...
  def choose_best_replacement_pokemon(idxBattler, terrible_moves = false)
    ...
    _, idxPartyEnd = @battle.pbTeamIndexRangeFromBattlerIndex(idxBattler)
    reserves = []
...
Battle::AI::Handlers::ShouldSwitch.add(:cure_status_problem_by_switching_out,
                                       proc { |battler, reserves, ai, battle|
                                         ...
                                         if battler.ability == :NATURALCURE || (single_status_cure && single_status_cure == battler.status)
                                           ...
                                           if battler.hp >= battler.totalhp / 2 && ![:SLEEP, :FROZEN].include?(battler.status)
                                             next false
                                           end
...
Battle::AI::Handlers::ShouldSwitch.add(:yawning,
                                       proc { |battler, reserves, ai, battle|
                                         ...
                                         if battler.ability_active?
                                           if [:INSOMNIA, :NATURALCURE, :REGENERATOR, :SHEDSKIN].include?(battler.ability_id)
                                             next false
                                           end
                                           if battler.ability_id == :HYDRATION && [:Rain, :HeavyRain].include?(battler.battler.effectiveWeather)
                                             next false
                                           end
                                         end
                                         if battler.has_active_item?([:CHESTOBERRY, :LUMBERRY]) && battler.battler.canConsumeBerry?
                                           next false
                                         end
