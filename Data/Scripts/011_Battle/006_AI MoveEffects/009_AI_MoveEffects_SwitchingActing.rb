...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("SwitchOutTargetStatusMove",
                                                       proc { |score, move, user, target, ai, battle|
                                                         ...
                                                         if ai.trainer.has_skill_flag?("HPAware") && (target.hp <= target.totalhp / 3)
                                                           score -= 10
                                                         end
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("BindTarget",
                                                       proc { |score, move, user, target, ai, battle|
                                                         ...
                                                         # effect)
                                                         if (target.can_become_trapped? || target.battler.takesIndirectDamage?) && (ai.trainer.medium_skill? &&
...
Battle::AI::Handlers::MoveFailureAgainstTargetCheck.add("TrapTargetInBattle",
                                                        proc { |move, user, target, ai, battle|
                                                          ...
                                                          if Settings::MORE_TYPE_EFFECTS && target.has_type?(:GHOST)
                                                            next true
                                                          end
                                                          next false
                                                        }
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("TargetActsLast",
                                                       proc { |score, move, user, target, ai, battle|
                                                         ...
                                                         has_ally = false
                                                         ai.each_ally(user.index) { |b, i|
                                                           has_ally = true if b.can_attack?
                                                         }
                                                         ...
                                                         idx_target = speeds.index { |ele| ele[0] == target.index }
                                                         idx_slowest_ally = -1
                                                         speeds.each_with_index { |ele, i|
                                                           idx_slowest_ally = i if user.index.even? == ele[0].even?
                                                         }
                                                         # Useless if the target is faster than the user
                                                         next Battle::AI::MOVE_USELESS_SCORE if idx_target < idx_user
                                                         # Useless if the target will move last anyway
                                                         if idx_target == speeds.length - 1
                                                           next Battle::AI::MOVE_USELESS_SCORE
                                                         end
                                                         # Useless if the slowest ally is faster than the target
                                                         if idx_slowest_ally < idx_target
                                                           next Battle::AI::MOVE_USELESS_SCORE
                                                         end
                                                         # Generally not worth using
                                                         next score - 10
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("TargetUsesItsLastUsedMoveAgain",
                                                       proc { |score, move, user, target, ai, battle|
                                                         ...
                                                         if mov.nil? || (mov.pp == 0 && mov.total_pp > 0)
                                                           next Battle::AI::MOVE_USELESS_SCORE
                                                         end
                                                         if move.move.moveBlacklist.include?(mov.function_code)
                                                           next Battle::AI::MOVE_USELESS_SCORE
                                                         end
                                          ...
                                          proc { |score, move, user, ai, battle|
                                            ...
                                            ai.each_battler do |b, i|
                                              if b.opposes?(user)
                                                ...
                                                if user.pbOpposingSide.effects[PBEffects::Tailwind] > 1
                                                  foe_speeds.last *= 2
                                                end
                                                if user.pbOpposingSide.effects[PBEffects::Swamp] > 1
                                                  foe_speeds.last /= 2
                                                end
                                              else
                                            ...
                                            if ally_speeds.length == 0 || foe_speeds.length == 0
                                              next Battle::AI::MOVE_USELESS_SCORE
                                            end
                                                       ...
                                                       proc { |score, move, user, target, ai, battle|
                                                         ...
                                                         if user.faster_than?(target)
                                                           ...
                                                           if last_move && last_move.total_pp > 0
                                                             score += add_effect
                                                             # Will fully deplete the move's PP
                                                             next score + 20 if last_move.pp <= 3
                                                             next score + 10 if last_move.pp <= 5
                                                             # Too much PP left to make a difference
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("LowerPPOfTargetLastMoveBy4",
                                                       proc { |score, move, user, target, ai, battle|
                                                         if user.faster_than?(target)
                                                           last_move = target.battler.pbGetMoveWithID(target.battler.lastRegularMoveUsed)
                                                           # Will fully deplete the move's PP
                                                           next score + 20 if last_move.pp <= 4
                                                           next score + 10 if last_move.pp <= 6
                                                           # Too much PP left to make a difference
                                                           next score - 10 if last_move.pp > 10
                                                         end
...
Battle::AI::Handlers::MoveFailureAgainstTargetCheck.add("DisableTargetLastMoveUsed",
                                                        proc { |move, user, target, ai, battle|
                                                          if target.effects[PBEffects::Disable] > 0 || !target.battler.lastRegularMoveUsed
                                                            next true
                                                          end
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("DisableTargetLastMoveUsed",
                                                       proc { |score, move, user, target, ai, battle|
                                                         if target.has_active_item?(:MENTALHERB)
                                                           next Battle::AI::MOVE_USELESS_SCORE
                                                         end
                                                         # Inherent preference
                                                         score += 5
                                                         ...
                                                         if GameData::Move.try_get(target.battler.lastRegularMoveUsed)&.damaging?
                                                           score += 8
                                                         end
                                                         next score
                                                       }
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("DisableTargetStatusMoves",
                                                       proc { |score, move, user, target, ai, battle|
                                                         if !target.check_for_move { |m| m.statusMove? }
                                                           next Battle::AI::MOVE_USELESS_SCORE
                                                         end
                                                         # Not worth using on a sleeping target that won't imminently wake up
                                                         if target.status == :SLEEP && target.statusCount > ((target.faster_than?(user)) ? 2 : 1) && !target.check_for_move { |m| m.statusMove? && m.usableWhenAsleep? }
                                                           next Battle::AI::MOVE_USELESS_SCORE
                                                           end
                                                         ...
                                                         if !target.effects[PBEffects::ChoiceBand] && (target.has_active_item?([:CHOICEBAND, :CHOICESPECS, :CHOICESCARF]) ||
                                                              target.has_active_ability?(:GORILLATACTICS))
                                                           next Battle::AI::MOVE_USELESS_SCORE
                                                           end
