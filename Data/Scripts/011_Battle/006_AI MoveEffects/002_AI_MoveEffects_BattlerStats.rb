...
Battle::AI::Handlers::MoveEffectScore.add("RaiseUserSpDef1PowerUpElectricMove",
                                          proc { |score, move, user, ai, battle|
                                            score = ai.get_score_for_target_stat_raise(score, user, move.move.statUp)
                                            score += 10 if user.has_damaging_move_of_type?(:ELECTRIC)
                                            next score
                                          }
                                         )
                                              ...
                                              if current_weight > 1
                                                ...
                                                ai.each_foe_battler(user.side) do |b, i|
                                                  ...
                                                  if b.has_move_with_function?("TwoTurnAttackInvulnerableInSkyTargetCannotAct") &&
                                                     Settings::MECHANICS_GENERATION >= 6 && (current_weight >= 2000 && current_weight < 3000)
                                                    score -= 10
                                                  end
                                                end
                                              end
...
Battle::AI::Handlers::MoveEffectScore.add("RaiseUserCriticalHitRate2",
                                          proc { |score, move, user, ai, battle|
                                            ...
                                            if ai.trainer.medium_skill?
                                              ...
                                              if user.item_active? && ([:RAZORCLAW, :SCOPELENS].include?(user.item_id) ||
...
Battle::AI::Handlers::MoveFailureCheck.add("RaiseUserAtkDef1",
                                           proc { |move, user, ai, battle|
                                             ...
                                             (move.move.statUp.length / 2).times do |i|
                                               if !user.battler.pbCanRaiseStatStage?(move.move.statUp[i * 2], user.battler, move.move)
                                                 next
                                               end
                                               will_fail = false
                                               break
...
Battle::AI::Handlers::MoveFailureCheck.add("LowerUserDefSpDef1RaiseUserAtkSpAtkSpd2",
                                           proc { |move, user, ai, battle|
                                             ...
                                             (move.move.statDown.length / 2).times do |i|
                                               if !user.battler.pbCanLowerStatStage?(move.move.statDown[i * 2], user.battler, move.move)
                                                 next
                                               end
                                               will_fail = false
                                               break
...
Battle::AI::Handlers::MoveEffectScore.add("StartRaiseUserAtk1WhenDamaged",
                                          proc { |score, move, user, ai, battle|
                                            ...
                                            if user.has_move_with_function?("PowerHigherWithUserPositiveStatStages")
                                              score += 7
                                            end
                                            next score
                                          }
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("RaiseTargetAttack2ConfuseTarget",
                                                       proc { |score, move, user, target, ai, battle|
                                                         if (!target.has_active_ability?(:CONTRARY) || battle.moldBreaker) && !target.battler.pbCanConfuse?(user.battler, false, move.move)
                                                           next Battle::AI::MOVE_USELESS_SCORE
                                                           end
...
Battle::AI::Handlers::MoveFailureAgainstTargetCheck.add("RaiseTargetRandomStat2",
                                                        proc { |move, user, target, ai, battle|
                                                          ...
                                                          GameData::Stat.each_battle do |s|
                                                            if !target.battler.pbCanRaiseStatStage?(s.id, user.battler, move.move)
                                                              next
                                                            end
                                                            will_fail = false
                                                          end
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("RaiseTargetRandomStat2",
                                                       proc { |score, move, user, target, ai, battle|
                                                         ...
                                                         # Less likely to use on ally
                                                         score -= 7 if target.index != user.index
                                                         ...
                                                         ai.each_foe_battler(target.side) do |b, i|
                                                           if !b.has_move_with_function?("PowerHigherWithTargetPositiveStatStages")
                                                             next
                                                           end
                                                           score -= 8
                                                         end
...
Battle::AI::Handlers::MoveFailureAgainstTargetCheck.add("LowerTargetSpAtk2IfCanAttract",
                                                        proc { |move, user, target, ai, battle|
                                                          ...
                                                          if !battle.moldBreaker && target.has_active_ability?(:OBLIVIOUS)
                                                            next true
                                                          end
                                                          next false
                                                        }
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("LowerTargetSpeed1MakeTargetWeakerToFire",
                                                       proc { |score, move, user, target, ai, battle|
                                                         ...
                                                         if !target.effects[PBEffects::TarShot]
                                                           ...
                                                           if !Effectiveness.ineffective?(eff) && user.has_damaging_move_of_type?(:FIRE)
                                                             score += 10 * eff
                                                           end
                                                         end
                                                         next score
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("LowerTargetEvasion1RemoveSideEffects",
                                                       proc { |score, move, user, target, ai, battle|
                                                         ...
                                                         if target.can_switch_lax? && (target.pbOwnSide.effects[PBEffects::Spikes] > 0 ||
                                                                          target.pbOwnSide.effects[PBEffects::ToxicSpikes] > 0 ||
                                                                          target.pbOwnSide.effects[PBEffects::StealthRock] ||
                                                                          target.pbOwnSide.effects[PBEffects::StickyWeb])
                                                           score -= 15
                                                         end
                                                         if user.can_switch_lax? && Settings::MECHANICS_GENERATION >= 6 && (target.pbOpposingSide.effects[PBEffects::Spikes] > 0 ||
...
Battle::AI::Handlers::MoveFailureAgainstTargetCheck.add("LowerTargetAtkDef1",
                                                        proc { |move, user, target, ai, battle|
                                                          ...
                                                          will_fail = true
                                                          (move.move.statDown.length / 2).times do |i|
                                                            if !target.battler.pbCanLowerStatStage?(move.move.statDown[i * 2], user.battler, move.move)
                                                              next
                                                            end
                                                            will_fail = false
                                                            break
...
Battle::AI::Handlers::MoveFailureAgainstTargetCheck.add("RaiseGroundedGrassBattlersAtkSpAtk1",
                                                        proc { |move, user, target, ai, battle|
                                                          if !target.has_type?(:GRASS) || target.battler.airborne? || target.battler.semiInvulnerable?
                                                            next true
                                                          end
                                                          next !target.battler.pbCanRaiseStatStage?(:ATTACK, user.battler, move.move) &&
                                                               !target.battler.pbCanRaiseStatStage?(:SPECIAL_ATTACK, user.battler, move.move)
...
Battle::AI::Handlers::MoveFailureAgainstTargetCheck.add("RaiseGrassBattlersDef1",
                                                        proc { |move, user, target, ai, battle|
                                                          if !target.has_type?(:GRASS) || target.battler.semiInvulnerable?
                                                            next true
                                                          end
                                                          next !target.battler.pbCanRaiseStatStage?(:DEFENSE, user.battler, move.move)
                                                        }
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("UserTargetSwapAtkSpAtkStages",
                                                       proc { |score, move, user, target, ai, battle|
                                                         ...
                                                         # No stat raises
                                                         next Battle::AI::MOVE_USELESS_SCORE if raises.length == 0
                                                         if raises.length > 0
                                                           score = ai.get_score_for_target_stat_raise(score, user, raises, false, true)
                                                         end
                                                         if raises.length > 0
                                                           score = ai.get_score_for_target_stat_drop(score, target, raises, false, true)
                                                         end
                                                         if drops.length > 0
                                                           score = ai.get_score_for_target_stat_drop(score, user, drops, false, true)
                                                         end
                                                         if drops.length > 0
                                                           score = ai.get_score_for_target_stat_raise(score, target, drops, false, true)
                                                         end
                                                         next score
                                                       }
                                                      )
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("InvertTargetStatStages",
                                                       proc { |score, move, user, target, ai, battle|
                                                         ...
                                                         # No stats will drop
                                                         next Battle::AI::MOVE_USELESS_SCORE if drops.length == 0
                                                         if raises.length > 0
                                                           score = ai.get_score_for_target_stat_raise(score, target, raises, false, true)
                                                         end
                                                         if drops.length > 0
                                                           score = ai.get_score_for_target_stat_drop(score, target, drops, false, true)
                                                         end
                                                         next score
                                                       }
                                                      )
...
Battle::AI::Handlers::MoveEffectScore.add("ResetAllBattlersStatStages",
                                          proc { |score, move, user, ai, battle|
                                            ai.each_battler do |b|
                                              ...
                                              if raises.length > 0
                                                score = ai.get_score_for_target_stat_raise(score, b, raises, false, true)
                                              end
                                              if drops.length > 0
                                                score = ai.get_score_for_target_stat_drop(score, b, drops, false, true)
                                              end
                                            end
                                            next score
                                          }
...
Battle::AI::Handlers::MoveEffectScore.add("StartUserSideImmunityToStatStageLowering",
                                          proc { |score, move, user, ai, battle|
                                            has_move = false
                                            ai.each_foe_battler(user.side) do |b, i|
                                              next unless b.check_for_move { |m|
...
Battle::AI::Handlers::MoveEffectScore.add("UserSwapBaseAtkDef",
                                          proc { |score, move, user, ai, battle|
                                            ...
                                            if user_def > user_atk   # Attack will be raised
                                              ...
                                              # No downside
                                              score += 5 if !ai.stat_drop_worthwhile?(user, :DEFENSE, true)
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("UserTargetAverageBaseAtkSpAtk",
                                                       proc { |score, move, user, target, ai, battle|
                                                         ...
                                                         if user_atk >= target_atk && user_spatk >= target_spatk
                                                           next Battle::AI::MOVE_USELESS_SCORE
                                                         end
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("UserTargetAverageBaseDefSpDef",
                                                       proc { |score, move, user, target, ai, battle|
                                                         ...
                                                         if user_def >= target_def && user_spdef >= target_spdef
                                                           next Battle::AI::MOVE_USELESS_SCORE
                                                         end
                                                         change_matters = false
...
Battle::AI::Handlers::MoveEffectScore.add("StartUserSideDoubleSpeed",
                                          proc { |score, move, user, ai, battle|
                                            ...
                                            if ai.trainer.medium_skill? && (battle.field.effects[PBEffects::TrickRoom] > 1)
                                              next Battle::AI::MOVE_USELESS_SCORE
                                              end
