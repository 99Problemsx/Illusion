...
Battle::AI::Handlers::MoveFailureAgainstTargetCheck.add("OHKO",
                                                        proc { |move, user, target, ai, battle|
                                                          ...
                                                          if !battle.moldBreaker && target.has_active_ability?(:STURDY)
                                                            next true
                                                          end
                                                          next false
                                                        }
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("OHKO",
                                                       proc { |score, move, user, target, ai, battle|
                                                         ...
                                                         if ai.trainer.has_skill_flag?("HPAware") && user.check_for_move { |m| m.damagingMove? && !m.is_a?(Battle::Move::OHKO) }
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("DamageTargetAlly",
                                                       proc { |score, move, user, target, ai, battle|
                                                         target.battler.allAllies.each do |b|
                                                           ...
                                                           score += 10
                                                           if ai.trainer.has_skill_flag?("HPAware") && (b.hp <= b.totalhp / 16)
                                                             score += 10
                                                           end
                                                         end
                                                         next score
                                          ...
                                          proc { |score, move, user, ai, battle|
                                            ...
                                            ai.each_foe_battler(user.side) do |b, i|
                                              ...
                                              if crit_stage >= 0 && crit_stage < 50
                                                ...
                                                if b.check_for_move { |m| m.pbCritialOverride(b.battler, user.battler) > 0 }
                                                  crit_stage = 99
                                                end
                                                crit_stage = [crit_stage, Battle::Move::CRITICAL_HIT_RATIOS.length - 1].min
                                              end
...
Battle::AI::Handlers::MoveFailureCheck.add("StartWeakenElectricMoves",
                                           proc { |move, user, ai, battle|
                                             if Settings::MECHANICS_GENERATION >= 6
                                               next battle.field.effects[PBEffects::MudSportField] > 0
                                             end
...
Battle::AI::Handlers::MoveEffectScore.add("StartWeakenElectricMoves",
                                          proc { |score, move, user, ai, battle|
                                            ...
                                            ai.each_foe_battler(user.side) do |b, i|
                                              ...
                                              score += 15
                                              if !b.check_for_move { |m| m.damagingMove? && m.pbCalcType(b.battler) != :ELECTRIC }
                                                score += 7
                                              end
...
Battle::AI::Handlers::MoveFailureCheck.add("StartWeakenFireMoves",
                                           proc { |move, user, ai, battle|
                                             if Settings::MECHANICS_GENERATION >= 6
                                               next battle.field.effects[PBEffects::WaterSportField] > 0
                                             end
...
Battle::AI::Handlers::MoveEffectScore.add("StartWeakenFireMoves",
                                          proc { |score, move, user, ai, battle|
                                            ...
                                            ai.each_foe_battler(user.side) do |b, i|
                                              ...
                                              score += 15
                                              if !b.check_for_move { |m| m.damagingMove? && m.pbCalcType(b.battler) != :FIRE }
                                                score += 7
                                              end
...
Battle::AI::Handlers::MoveEffectScore.add("StartWeakenPhysicalDamageAgainstUserSide",
                                          proc { |score, move, user, ai, battle|
                                            ...
                                            # Don't prefer the lower the user's HP is
                                            if ai.trainer.has_skill_flag?("HPAware") && battle.pbAbleNonActiveCount(user.idxOwnSide) == 0 && (user.hp <= user.totalhp / 2)
                                              score -= (20 * (0.75 - (user.hp.to_f / user.totalhp))).to_i   # -5 to -15
                                              end
...
Battle::AI::Handlers::MoveEffectScore.add("ProtectUserSideFromStatusMoves",
                                          proc { |score, move, user, ai, battle|
                                            ...
                                            ai.each_foe_battler(user.side) do |b, i|
                                              next if !b.can_attack?
                                              next if !b.check_for_move { |m|
                                                        m.statusMove? && m.pbTarget(b.battler).targets_foe &&
...
Battle::AI::Handlers::MoveEffectScore.add("ProtectUserSideFromPriorityMoves",
                                          proc { |score, move, user, ai, battle|
                                            ...
                                            ai.each_foe_battler(user.side) do |b, i|
                                              next if !b.can_attack?
                                              if !b.check_for_move { |m| m.pbPriority(b.battler) > 0 && m.canProtectAgainst? }
                                                next
                                              end
                                              useless = false
                                              # General preference
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("RemoveProtections",
                                                       proc { |score, move, user, target, ai, battle|
                                                         if target.check_for_move { |m|
                                                              (m.is_a?(Battle::Move::ProtectMove) ||
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("EnsureNextMoveAlwaysHits",
                                                       proc { |score, move, user, target, ai, battle|
                                                         if user.has_active_ability?(:NOGUARD) || target.has_active_ability?(:NOGUARD)
                                                           next Battle::AI::MOVE_USELESS_SCORE
                                                         end
                                                         ...
                                                         user.battler.eachMove do |m|
                                                           if target.effects[PBEffects::Minimize] && m.tramplesMinimize? && Settings::MECHANICS_GENERATION >= 6
                                                             next
                                                           end
                                                           acc = m.accuracy
                                                           if ai.trainer.medium_skill?
                                                             acc = m.pbBaseAccuracy(user.battler, target.battler)
                                                           end
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("StartNegateTargetEvasionStatStageAndGhostImmunity",
                                                       proc { |score, move, user, target, ai, battle|
                                                         if target.effects[PBEffects::Foresight] || user.has_active_ability?(:SCRAPPY)
                                                           next Battle::AI::MOVE_USELESS_SCORE
                                                         end
                                                         ...
                                                         if target.has_type?(:GHOST)
                                                           user.battler.eachMove do |m|
                                                             ...
                                                             if Effectiveness.ineffective_type?(m.pbCalcType(user.battler), :GHOST)
                                                               score += 10
                                                             end
                                                           end
                                                         end
                                                         # Prefer if the target has increased evasion
                                                         if target.stages[:EVASION] > 0
                                                           score += 10 * target.stages[:EVASION]
                                                         end
                                                         next score
                                                       }
                                                      )
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("StartNegateTargetEvasionStatStageAndDarkImmunity",
                                                       proc { |score, move, user, target, ai, battle|
                                                         ...
                                                         if target.has_type?(:DARK)
                                                           user.battler.eachMove do |m|
                                                             next if !m.damagingMove?
                                                             if Effectiveness.ineffective_type?(m.pbCalcType(user.battler), :DARK)
                                                               score += 10
                                                             end
                                                           end
                                                         end
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("TargetMovesBecomeElectric",
                                                       proc { |score, move, user, target, ai, battle|
                                                         ...
                                                         if user.has_active_ability?([:LIGHTNINGROD, :MOTORDRIVE, :VOLTABSORB])
                                                           electric_eff = 0
                                                         end
                                                         ...
                                                         target.battler.eachMove do |m|
                                                           ...
                                                           when :WATER
                                                             if user.has_active_ability?([:STORMDRAIN, :WATERABSORB])
                                                               eff = 0
                                                             end
                                                           end
                                                           if eff > electric_eff
                                                         ...
                                                         if electric_type_better == 0
                                                           next Battle::AI::MOVE_USELESS_SCORE
                                                         end
                                                         if electric_type_better < old_type_better
                                                           next Battle::AI::MOVE_USELESS_SCORE
                                                         end
