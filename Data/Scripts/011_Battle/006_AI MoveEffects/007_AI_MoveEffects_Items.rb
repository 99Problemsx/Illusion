...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("UserTakesTargetItem",
                                                       proc { |score, move, user, target, ai, battle|
                                                         ...
                                                         if target.has_active_ability?(:STICKYHOLD) && !battle.moldBreaker
                                                           next score
                                                         end
...
Battle::AI::Handlers::MoveFailureAgainstTargetCheck.add("TargetTakesUserItem",
                                                        proc { |move, user, target, ai, battle|
                                                          if !user.item || user.battler.unlosableItem?(user.item)
                                                            next true
                                                          end
                                                          if target.item || target.battler.unlosableItem?(user.item)
                                                            next true
                                                          end
                                                          next false
                                                        }
...
Battle::AI::Handlers::MoveFailureAgainstTargetCheck.add("UserTargetSwapItems",
                                                        proc { |move, user, target, ai, battle|
                                                          ...
                                                          if user.battler.unlosableItem?(user.item) || user.battler.unlosableItem?(target.item)
                                                            next true
                                                          end
                                                          if target.battler.unlosableItem?(target.item) || target.battler.unlosableItem?(user.item)
                                                            next true
                                                          end
                                                          if target.has_active_ability?(:STICKYHOLD) && !battle.moldBreaker
                                                            next true
...
Battle::AI::Handlers::MoveFailureAgainstTargetCheck.add("CorrodeTargetItem",
                                                        proc { |move, user, target, ai, battle|
                                                          ...
                                                          if battle.corrosiveGas[target.index % 2][target.party_index]
                                                            next true
                                                          end
                                                          next false
                                                        }
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("StartTargetCannotUseItem",
                                                       proc { |score, move, user, target, ai, battle|
                                                         if !target.item || !target.item_active?
                                                         ...
                                                         # Item has no effect or is bad
                                                         next Battle::AI::MOVE_USELESS_SCORE if item_score <= 0
                                                         score += item_score * 2
                                                         next score
                                                       }
                                                      )
...
Battle::AI::Handlers::MoveEffectScore.add("StartNegateHeldItems",
                                          proc { |score, move, user, ai, battle|
                                            ...
                                            next if battle.field.effects[PBEffects::MagicRoom] == 1
...
Battle::AI::Handlers::MoveEffectScore.add("UserConsumeBerryRaiseDefense2",
                                          proc { |score, move, user, ai, battle|
                                            ...
                                            if stat_raise_score != Battle::AI::MOVE_USELESS_SCORE
                                              score += stat_raise_score
                                            end
                                            ...
                                            if ai.trainer.medium_skill?
                                              ...
                                              if !user.battler.belched? && user.has_move_with_function?("FailsIfUserNotConsumedBerry")
                                                score += 5
                                              end
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("AllBattlersConsumeBerry",
                                                       proc { |score, move, user, target, ai, battle|
                                                         ...
                                                         if ai.trainer.medium_skill?
                                                           ...
                                                           if !target.battler.belched? && target.has_move_with_function?("FailsIfUserNotConsumedBerry")
                                                             score_change += 5
                                                           end
...
Battle::AI::Handlers::MoveFailureCheck.add("ThrowUserItemAtTarget",
                                           proc { |move, user, ai, battle|
                                             item = user.item
                                             if !item || !user.item_active? || user.battler.unlosableItem?(item)
                                               next true
                                             end
