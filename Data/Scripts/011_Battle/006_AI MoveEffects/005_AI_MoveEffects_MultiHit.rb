...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("HitTwoTimes",
                                                       proc { |score, move, user, target, ai, battle|
                                                         ...
                                                         if target.effects[PBEffects::Substitute] > 0 && !move.move.ignoresSubstitute?(user.battler)
                                                           ...
                                                           if target.effects[PBEffects::Substitute] < dmg * (num_hits - 1) / num_hits
                                                             score += 10
                                                           end
                                                         end
                                                         next score
...
Battle::AI::Handlers::MoveEffectAgainstTargetScore.add("TwoTurnAttack",
                                                       proc { |score, move, user, target, ai, battle|
                                                         ...
                                                         if user.has_active_ability?(:TRUANT)
                                                         ...
                                                         if ai.trainer.high_skill? && !(user.has_active_ability?(:UNSEENFIST) && move.move.contactMove?)
                                                           ...
                                                           if move.pbTarget(user).num_targets > 1 &&
                                                              (Settings::MECHANICS_GENERATION >= 7 || move.damagingMove?) && target.has_move_with_function?("ProtectUserSideFromMultiTargetDamagingMoves")
                                                           ...
                                                           if move.move.canProtectAgainst?
                                                             ...
                                                             if move.damagingMove? && target.has_move_with_function?("ProtectUserFromDamagingMovesKingsShield",
                                                                                                                     "ProtectUserFromDamagingMovesObstruct")
                                                               has_protect_move = true
                                                                                                                            end
                                                             if move.rough_priority(user) > 0 && target.has_move_with_function?("ProtectUserSideFromPriorityMoves")
                                                               has_protect_move = true
                                                               end
                                                           end
...
Battle::AI::Handlers::MoveFailureAgainstTargetCheck.add("TwoTurnAttackInvulnerableInSkyTargetCannotAct",
                                                        proc { |move, user, target, ai, battle|
                                                          ...
                                                          if Settings::MECHANICS_GENERATION >= 6 && target.battler.pbWeight >= 2000
                                                            next true
                                                          end
                                                          if target.battler.semiInvulnerable? || target.effects[PBEffects::SkyDrop] >= 0
                                                            next true
                                                          end
                                                          next false
                                                        }
