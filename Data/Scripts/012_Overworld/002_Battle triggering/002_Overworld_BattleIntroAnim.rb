...
SpecialBattleIntroAnimations.register("vs_trainer_animation", 60,   # Priority 60
                                      proc { |battle_type, foe, location|   # Condition
                                        # Trainer battle against 1 trainer
...
SpecialBattleIntroAnimations.register("alternate_vs_trainer_animation", 50,   # Priority 50
                                      ...
                                      proc { |viewport, battle_type, foe, location|   # Animation
                                        ...
                                        tr_type = foe[0].trainer_type
                                        trainer_bar_graphic = begin
                                          sprintf("vsBar_%s", tr_type.to_s)
                                        rescue StandardError
                                          nil
                                        end
                                        trainer_graphic = begin
                                          sprintf("vsTrainer_%s", tr_type.to_s)
                                        rescue StandardError
                                          nil
                                        end
                                        player_tr_type = $player.trainer_type
                                        outfit = $player.outfit
                                        player_bar_graphic = begin
                                          sprintf("vsBar_%s_%d", player_tr_type.to_s, outfit)
                                        rescue StandardError
                                          nil
                                        end
                                        if !pbResolveBitmap("Graphics/Transitions/" + player_bar_graphic)
                                          player_bar_graphic = begin
                                            sprintf("vsBar_%s", player_tr_type.to_s)
                                          rescue StandardError
                                            nil
                                          end
                                        end
                                        player_graphic = begin
                                          sprintf("vsTrainer_%s_%d", player_tr_type.to_s, outfit)
                                        rescue StandardError
                                          nil
                                        end
                                        if !pbResolveBitmap("Graphics/Transitions/" + player_graphic)
                                          player_graphic = begin
                                            sprintf("vsTrainer_%s", player_tr_type.to_s)
                                          rescue StandardError
                                            nil
                                          end
                                        end
