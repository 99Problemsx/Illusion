...
Battle::AI::Handlers::AbilityRanking.add(:ORICHALCUMPULSE,
                                         proc { |ability, score, battler, ai|
                                           if battler.check_for_move { |m| m.physicalMove? && [:Sun, :HarshSun].include?(battler.battler.effectiveWeather) }
                                             next score
    end
                                           next score - 1
                                         }
...
Battle::AI::Handlers::AbilityRanking.add(:HADRONENGINE,
                                         proc { |ability, score, battler, ai|
                                           if battler.check_for_move { |m| m.specialMove? && battler.battler.battle.field.terrain == :Electric }
                                             next score
                                           end
                                           next score - 1
                                         }
