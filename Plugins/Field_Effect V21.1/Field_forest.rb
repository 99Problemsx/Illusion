  ...
  def initialize(battle, duration = Battle::Field::DEFAULT_FIELD_DURATION)
    ...
    @multipliers = {
      [:power_multiplier, 1.5, _INTL("The forestry strengthened the attack!")] => proc { |user, target, numTargets, move, type, power, mults|
        next true if %i[GRASS].include?(type) && user.grounded?
      },
...
# Special Bug-type moves increase in base power by 1.5x. #
# Effect Spore's activation chance is doubled to 60%. #
# Grass Pelt, Leaf Guard, Overgrow, and Swarm are activated. #
...
# Shelter halves damage from Bug-type moves. #
# Terrain Pulse's type becomes Bug. #
