...
def set_field(new_field = nil, duartion = Battle::Field::INFINITE_FIELD_DURATION )
  $field = [new_field, duartion]
end
...
=end
  ...
  # the test field now is Electric, field duartion is 3
  def set_test_field(test_field = :electric, duration = 3)
  ...
  def create_new_field(id, duration = Battle::Field::DEFAULT_FIELD_DURATION, bg_change: true)
    return unless id
    return if try_create_zero_duration_field?(duration)

    formatted_field_id = id.to_s.downcase
  ...
  def remove_field(remove_field: nil, ignore_infinite: true, remove_all: false)
    ...
    if remove_field
      if ignore_infinite
        ...
        removed_field = nil
        @stacked_field.delete_if do |field|
