...
class Sprite
  ...
  def center!(snap = false)
    ...
    return unless snap && self.viewport
  ...
  def create_outline(color, thickness = 2)
    ...
    (0...self.bitmap.width).each do |x|
      # iterates through each Y coordinate
      (0...self.bitmap.height).each do |y|
  ...
  #-----------------------------------------------------------------------------
  # Draw sprite on a sprite/bitmap
...
class ScrollingSprite < Sprite
  ...
  def update
    ...
    mod = [@direction, ([@speed, 1].max * @direction).delta_sub(false)]
    if @vertical
    ...
    if @pulse
      self.opacity -= (@gopac * [@speed, 1].max).delta_sub(false)
      @gopac *= -1 if [@max_o, @min_o].include?(self.opacity)
    end
    @frame = 0
  end
  #-----------------------------------------------------------------------------
...
class TrailingSprite
  ...
  def update
    ...
    @sprites.each_key do |key|
      next unless @sprites[key].opacity > @keyFrame.delta_add(false)
...
class SelectorSprite < SpriteSheet
  ...
  def render(rect, file = nil, vertical = false)
    ...
    @filename if file.nil? && !@filename.nil?
    @curFrame = 0
    self.src_rect.x = 0
    self.src_rect.y = 0
        ...
        (0...a[1].length).each do |q|
          ...
          begin
            if a[1][q].is_numeric? && a[1][q].include?(".")
              ...
              typ = "Integer"
              a[1][q] = a[1][q].to_i
            elsif ["true", "false"].include?(a[1][q].downcase)
              typ = "Boolean"
